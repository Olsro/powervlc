// Generated from crt/shaders/crt-sines.slang. See slang/upstream for licence/source.
cbuffer UBO : register(b0)
{
    row_major float4x4 global_MVP : packoffset(c0);
    float4 global_SourceSize : packoffset(c4);
    float4 global_OutputSize : packoffset(c6);
};

cbuffer Push : register(b1)
{
    float params_MSK_WIDTH : packoffset(c4);
    float params_MSK_HEIGHT : packoffset(c4.y);
};


static float4 gl_Position;
static float4 Position;
static float2 vTexCoord;
static float2 TexCoord;
static float2 invDims;
static float2 maskpos;

struct SPIRV_Cross_Input
{
    float4 Position : TEXCOORD0;
    float2 TexCoord : TEXCOORD1;
};

struct SPIRV_Cross_Output
{
    float2 vTexCoord : TEXCOORD0;
    float2 invDims : TEXCOORD1;
    float2 maskpos : TEXCOORD2;
    float4 gl_Position : SV_Position;
};

void vert_main()
{
    gl_Position = mul(Position, global_MVP);
    vTexCoord = TexCoord * 1.00010001659393310546875f;
    invDims = 1.0f.xx / global_SourceSize.xy;
    maskpos = (vTexCoord * global_OutputSize.xy) / float2(params_MSK_WIDTH, params_MSK_HEIGHT);
}

SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
{
    Position = stage_input.Position;
    TexCoord = stage_input.TexCoord;
    vert_main();
    SPIRV_Cross_Output stage_output;
    stage_output.gl_Position = gl_Position;
    stage_output.vTexCoord = vTexCoord;
    stage_output.invDims = invDims;
    stage_output.maskpos = maskpos;
    return stage_output;
}
