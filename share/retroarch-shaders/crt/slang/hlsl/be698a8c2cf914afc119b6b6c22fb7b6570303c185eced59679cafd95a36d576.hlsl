...
void vert_main()
{
    ...
    blur_dxdy = (params_SourceSize.xy / params_OutputSize.xy) * texture_size_inv;
    ...
    do
    {
        if (global_interlace_detect_toggle != 0.0f)
        {
            ...
            {
                _402 = (params_SourceSize.y > 1079.5f) && (params_SourceSize.y < 1080.5f);
            }
            else
            {
                _402 = false;
            }
            _404 = ((params_SourceSize.y > 288.5f) && (params_SourceSize.y < 576.5f)) || _402;
            break;
        }
        else
        {
    ...
    uv_scanline_step = float2(1.0f, _404 ? 2.0f : 1.0f) / params_SourceSize.xy;
}
