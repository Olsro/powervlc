...
cbuffer Push : register(b1)
{
    ...
    float4 params_MASKED_SCANLINESSize : packoffset(c4);
    float4 params_BLOOM_APPROXSize : packoffset(c5);
};
...
void vert_main()
{
    ...
    blur3x3_tex_uv = TexCoord;
    float2 _545;
    do
    {
        float _475 = 8.0f * lerp(global_mask_triad_size_desired, params_OutputSize.x / global_mask_num_triads_desired, global_mask_specify_num_triads);
        if (global_mask_sample_mode_desired > 0.5f)
        {
            _545 = 1.0f.xx * _475;
            break;
        }
        float2 _500 = clamp(1.0f.xx * min(_475, 64.0f), 1.0f.xx * ceil(16.0f), params_OutputSize.xy * 0.03125f.xx);
        _545 = floor(float2(min(_500.x, _500.y), _570) + 1.52587890625e-05f.xx);
        break;
    } while(false);
    bloom_sigma_runtime = ((-0.0516799986362457275390625f) + (_545.x * 0.076412498950958251953125f)) - (_545.x * 0.0092205703258514404296875f);
}
