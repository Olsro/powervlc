...
void main()
{
    ivec2 _246 = ivec2(floor(RA_VARYING_0 * (vec4(Pass4TextureSize, 1.0 / Pass4TextureSize)).xy));
    vec3 _262 = mix(texelFetch(Pass4Texture, _246, 0).xyz, textureLod(Texture, RA_VARYING_0, 0.0).xyz, vec3((GlowAmount)));
    vec3 _476;
    if ((OutputGamma) < 0.5)
    {
