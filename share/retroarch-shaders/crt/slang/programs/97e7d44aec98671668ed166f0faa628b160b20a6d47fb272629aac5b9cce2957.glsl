...
void main()
{
    ...
    float _66 = _63 * _63;
    float _70 = _66 * _66;
    vec3 _105 = texture2D(Texture, _46).xyz;
    vec3 _146 = _105 * mix(((BRIGHTBOOST) - ((LOWLUMSCAN) * (_66 - (2.0499999523162841796875 * _70)))) * (1.0 + (float(fract(floor(RA_VARYING_0.x * (vec4(OutputSize, 1.0 / OutputSize)).x) * (-0.4999000132083892822265625)) < 0.5) * (-(MASK_DARK)))), 1.0 - ((HILUMSCAN) * (_70 - ((2.7999999523162841796875 * _70) * _66))), dot(_105, vec3(RA_VARYING_1)));
    gl_FragData[0].x = _146.x;
    gl_FragData[0].y = _146.y;
    gl_FragData[0].z = _146.z;
}
