...
void main()
{
    ...
    float _33 = mix(1.0, 0.300000011920928955078125, max(_19.y, _19.z));
    gl_FragData[0] = pow(texture2D(MASK, fract((RA_VARYING_0 * (vec4(OutputSize, 1.0 / OutputSize)).xy) / vec2(2.0, floor(((vec4(OutputSize, 1.0 / OutputSize)).y / (vec4(TextureSize, 1.0 / TextureSize)).y) + 9.9999999747524270787835121154785e-07)))), vec4(_33)) * (_19 * mix(1.0, 1.2000000476837158203125, _33));
}
