                                               ...
                                               const video_format_t *format)
{
    ...
    msg_Info(gl, "Dolby Vision renderer: libplacebo %s (profile %u%s)",
...
int vlc_dovi_renderer_Display(vlc_dovi_renderer_t *sys)
{
    ...
    if (swapframe.fbo->params.format->type == PL_FMT_FLOAT)
    {
        ...
        {
            msg_Info(sys->gl, "Dolby Vision output: floating-point EDR, %.0f-nit target",
                     sys->target_peak);
    ...
    {
        msg_Warn(sys->gl, "Dolby Vision output: integer SDR framebuffer; tone mapping is active");
        sys->reported_output = true;
    }
