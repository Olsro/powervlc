...
struct bluray_disc_t
{
    ...
    /* Read-ahead window, page aligned so the transfer never straddles in a way
     * the driver has to bounce. cache_lba is -1 when nothing is cached. */
    uint8_t                     *cache;
    int                          cache_lba;
    int                          cache_count;
...
bluray_disc_t *bluray_disc_OpenMMC(vlc_object_t *obj, const char *psz_bsd_name)
{
    ...
    p->cache = valloc(CACHE_BLOCKS * BD_BLOCK);
...
int bluray_disc_ReadBlocks(void *handle, void *buf, int lba, int num_blocks)
{
    ...
    while (done < num_blocks) {
        ...
        if (p->cache_lba < 0
         || want < p->cache_lba
         || want >= p->cache_lba + p->cache_count) {
            int got = FetchBlocks(p, want, CACHE_BLOCKS, p->cache);
            if (got <= 0) {
                p->cache_lba = -1;
                break;
            }
        ...
        int offset = want - p->cache_lba;
        int avail  = p->cache_count - offset;
        int take   = num_blocks - done;
        if (take > avail)
            take = avail;

        memcpy(out + done * BD_BLOCK, p->cache + offset * BD_BLOCK,
