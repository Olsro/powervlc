...
@interface QTCaptureDecompressedVideoOutput (VLCQTKitSnowLeopard)
- (void)setAutomaticallyDropsLateVideoFrames:(BOOL)drop;
- (void)setMinimumVideoFrameInterval:(NSTimeInterval)interval;
@end
...
- (void)outputVideoFrame:(CVImageBufferRef)videoFrame withSampleBuffer:(QTSampleBuffer *)sampleBuffer fromConnection:(QTCaptureConnection *)connection
{
    ...
    @synchronized (self) {
        ...
        /* QTKit 7.2 on Tiger supplies QTSampleBuffer instances that do not
         * implement all methods declared by the 10.4u SDK headers. Calling
         * -presentationTime (or -attributeForKey:) unconditionally raises an
         * exception on QTKit's capture thread. Fall back to VLC's monotonic
         * clock there; newer QTKit versions retain their native timestamps. */
        currentPts = mdate();
        timeScale = CLOCK_FREQ;
        if ([sampleBuffer respondsToSelector:@selector(presentationTime)]) {
            QTTime timeStamp = [sampleBuffer presentationTime];
            if (timeStamp.timeScale > 0) {
                timeScale = timeStamp.timeScale;
                currentPts = (mtime_t)((int64_t)CLOCK_FREQ
                           * timeStamp.timeValue / timeStamp.timeScale);
            }
        }

        /* Host time was added in Leopard. It also fixes broken iSight PTS,
         * but both the accessor and NSNumber's 64-bit getter must exist. */
        if ([sampleBuffer respondsToSelector:@selector(attributeForKey:)]) {
            NSNumber *hosttime = (NSNumber *)[sampleBuffer
                attributeForKey:@"QTSampleBufferHostTimeAttribute"];
            if (hosttime != nil
             && [hosttime respondsToSelector:@selector(unsignedLongLongValue)])
                currentPts = (mtime_t)AudioConvertHostTimeToNanos(
                    [hosttime unsignedLongLongValue]) / 1000;
        }
    }
...
static int Open(vlc_object_t *p_this)
{
    ...
    msg_Dbg(p_demux, "qtcapture uid = %s", psz_uid ? psz_uid : "");
    NSString *qtk_currdevice_uid = [NSString stringWithUTF8String:
        psz_uid ? psz_uid : ""];
    ...
    NSArray *myVideoDevices = [[[QTCaptureDevice inputDevicesWithMediaType:QTMediaTypeVideo]
        arrayByAddingObjectsFromArray:[QTCaptureDevice inputDevicesWithMediaType:@"muxx"]] retain];
    if ([myVideoDevices count] == 0) {
        vlc_dialog_display_error(p_demux, _("No Input device found"), "%s",
    ...
    NSSize encoded_size, display_size, par_size;
    ...
    [p_sys->output setPixelBufferAttributes: [NSDictionary dictionaryWithObjectsAndKeys:
                                              [NSNumber numberWithInt:'2vuy'], (id)kCVPixelBufferPixelFormatTypeKey,
    ...
    if ([p_sys->output
        ...
        [p_sys->output setMinimumVideoFrameInterval:(1.0 / 25.0)];
