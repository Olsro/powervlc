...
typedef struct
{
    struct dav_entry *data;
    size_t size;
    size_t capacity;
} dav_entry_vector_t;

static bool dav_vector_push(dav_entry_vector_t *v, struct dav_entry entry)
{
    if (v->size == v->capacity)
    {
        size_t capacity = v->capacity ? v->capacity * 2 : 16;
        struct dav_entry *data = realloc(v->data, capacity * sizeof(*data));
        if (data == NULL)
            return false;
        v->data = data;
        v->capacity = capacity;
...
#define vlc_vector_push(v, e) dav_vector_push((v), (e))
#define vlc_vector_foreach_ref(pos, v) \
    for (size_t dav_i = 0; dav_i < (v)->size \
         && (((pos) = &(v)->data[dav_i]) != NULL); dav_i++)
#define vlc_vector_destroy(v) do { free((v)->data); (v)->data = NULL; \
                                   (v)->size = (v)->capacity = 0; } while (0)
...
static char *DavAuthority(const char *host, unsigned port)
{
    static const char *const formats[4] = { "%s", "[%s]", "%s:%u", "[%s]:%u" };
    const bool brackets = strchr(host, ':') != NULL;
    char *authority;
    if (asprintf(&authority, formats[brackets + 2 * (port != 0)],
                 host, port) < 0)
        return NULL;
    return authority;
}
                      ...
                      const struct dav_entry *e)
{
    ...
    vlc_readdir_helper_additem(rdh, url, NULL, name, type, ITEM_NET);
 ...
 *   </D:multistatus>
 *
 * The DAV: namespace prefix varies between servers. VLC 3's XML reader
 * exposes qualified names, so compare their local part after any prefix.
 * With Depth: 1 the first <response> is the request target
                            ...
                            bool *self_is_collection_out)
{
    ...
    while ((type = xml_ReaderNextNode(reader, &node)) > 0)
    {
...
static int Open(vlc_object_t *obj)
{
    ...
    sys->authority = DavAuthority(sys->url.psz_host, sys->url.i_port);
    ...
    vlc_credential_get(&crd, obj, NULL, NULL, NULL, NULL);
    const char *user = crd.psz_username;
    const char *pass = crd.psz_password;
    ...
    for (;;)
    {
        ...
        bool ok = vlc_credential_get(&crd, obj, NULL, NULL,
